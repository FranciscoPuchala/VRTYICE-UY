...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // ===== ANIMACIÓN DE ENTRADA PARA MODULE CARDS =====
    const moduleCardsObserver = new IntersectionObserver(function(entries) {
        entries.forEach((entry, index) => {
            if (entry.isIntersecting) {
                ...
                moduleCardsObserver.unobserve(entry.target);
            }
        });
    }, { threshold: 0.1 });
    ...
    moduleCards.forEach(card => {
        ...
        moduleCardsObserver.observe(card);
    });
    ...
    // ===== CONSOLE LOG =====
    console.log('Vértice Plant Applications - Initialized successfully! 🚀');
    console.log('Module cards:', moduleCards.length);
});
